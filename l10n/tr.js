OC.L10N.register(
    "files_antivirus",
    {
    "Antivirus" : "Antivirus",
    "File {file} is infected with {virus}" : "{file} dosyasına {virus} virusu bulaşmış",
    "The file has been removed" : "Dosya silindi",
    "File containing {virus} detected" : "Dosyada {virus} virüsü bulundu",
    "Antivirus detected a virus" : "Antivirus bir virus algıladı",
    "Virus %s is detected in the file. Upload cannot be completed." : "Dosyada %s virüsü bulundu. Yükleme tamamlanamadı.",
    "%s. Upload cannot be completed." : "%s. Yükleme tamamlanamadı.",
    "Saved" : "Kaydedildi",
    "Antivirus for files" : "Dosyalar için antivirus",
    "An antivirus app for Nextcloud" : "Nextcloud antivirus uygulaması",
    "Antivirus for files is an antivirus app for Nextcloud.\n\n* 🕵️‍♂️ When the user uploads a file, it's checked\n* ☢️ Uploaded and infected files will be deleted and a notification will be shown and/or sent via email\n* 🔎 Background Job to scan all files\n* ❓ Use ClamAV (open source), Kaspersky Scan Engine or an ICAP compatible scanner\n\nThis application inspects files that are uploaded to Nextcloud for viruses before they are written to the Nextcloud storage. If a file is identified as a virus, it is either logged or not uploaded to the server. The application relies on the underlying ClamAV virus scanning engine, which the admin points Nextcloud to when configuring the application. Alternatively, a Kaspersky Scan Engine can be configured, which has to run on a separate server.\nFor this app to be effective, the ClamAV virus definitions should be kept up to date. Also note that enabling this app will impact system performance as additional processing is required for every upload. More information is available in the Antivirus documentation." : "Dosyalar için antivirus, Nexctloud antivirus uygulamasıdır.\n\n* 🕵️‍♂️ Kullanıcıların yüklediği dosyaları denetler\n* ☢️ Yüklenmiş virüslü dosyaları siler ve bir bildirimi görüntüler ya da e-posta ile gönderir\n* 🔎 Arka planda tüm dosyaları tarayan bir görev yürütür\n* ❓ ClamAV (açık kaynaklı), Kaspersky tarama sistemi ya da ICAP uyumlu bir tarayıcı kullanır\n\nBu uygulama Nextcloud üzerine yüklenen dosyaları Nextcloud depolama alanı üzerine kaydetmeden önce virüslere karşı tarar. Bir dosyada virüs bulunursa, günlüğe kaydedilir ya da sunucuya yüklenmez. Bu uygulama Nextcloud yapılandırması sırasında yönetici tarafından belirtilen ClamAV virus tarama uygulamasını kullanır. Alternatif olarak, ayrı bir sunucuda çalıştırılması gereken Kaspersky tarama sistemi yapılandırılabilir\nBu uygulamanın etkili olması için ClamAV virus tanımlarının güncel olması gerekir. Ayrıca bu uygulama kullanıma alındığında, yüklenecek her dosya için ek işlemler yapılması nedeniyle sistem başarımı olumsuz etkilenebilir. Ayrıntılı bilgi almak için Antivirus uygulamasının belgelerine bakabilirsiniz",
    "Failed to load rules" : "Kurallar yüklenemedi",
    "Rule saved" : "Kural kaydedildi",
    "Failed to save rule" : "Kural kaydedilemedi",
    "Rule deleted" : "Kural silindi",
    "Failed to delete rule" : "Kural silinemedi",
    "All rules cleared" : "Tüm kurallar temizlendi",
    "Failed to clear rules" : "Kurallar temizlenemedi",
    "Rules reset to defaults" : "Kurallar varsayılana sıfırlandı",
    "Failed to reset rules" : "Kurallar sıfırlanamadı",
    "Advanced settings" : "Gelişmiş ayarlar",
    "Rules" : "Kurallar",
    "Clear all" : "Tümünü temizle",
    "Reset to defaults" : "Varsayılanlara dön",
    "Add a rule" : "Kural ekle",
    "Edit rule" : "Kuralı düzenle",
    "Match by" : "Şununla eşleşen",
    "Mark as" : "Şu olarak işaretlensin",
    "Scanner exit status" : "Tarayıcı çıkış durumu",
    "Signature to search" : "Aranacak imza",
    "Description" : "Açıklama",
    "Create" : "Oluştur",
    "Save" : "Kaydet",
    "Scanner exit status or signature to search" : "Tarayıcı çıkış durumu ya da arama imzası",
    "Actions" : "İşlemler",
    "Edit" : "Düzenle",
    "Delete" : "Sil",
    "ClamAV Executable" : "ClamAV çalıştırılabilir dosyası",
    "ClamAV Daemon" : "ClamAV arka plan işlemi",
    "ClamAV Socket" : "ClamAV soketi",
    "Only log" : "Yalnızca günlük kaydı tutulsun",
    "Delete file" : "Dosya silinsin",
    "Settings saved successfully" : "Ayarlar kaydedildi",
    "Failed to save settings" : "Ayarlar kaydedilemedi",
    "Mode" : "Kip",
    "Path to clamscan" : "Clamscan yolu",
    "Extra command line options" : "Ek komut satırı seçenekleri",
    "Extra command line options (comma-separated)" : "Ek komut satırı seçenekleri (virgül ile ayırarak yazın)",
    "Socket" : "Soket",
    "ClamAV Socket." : "ClamAV soketi.",
    "Host" : "Sunucu",
    "Address of Antivirus Host." : "Antivirus sunucusunun adresi",
    "Port" : "Bağlantı noktası",
    "Port number of Antivirus Host." : "Antivirus sunucusunun bağlantı noktası.",
    "ICAP preset" : "ICAP hazır ayarı",
    "Select" : "Seçin",
    "ICAP mode" : "ICAP kipi",
    "ICAP service" : "ICAP hizmeti",
    "ICAP virus response header" : "ICAP virus yanıtı üst bilgisi",
    "Use TLS encryption." : "TLS şifrelemesi kullanılır.",
    "Streaming options" : "Akış seçenekleri",
    "File size limit" : "Dosya boyutu sınırı",
    "File size limit for periodic background scans and chunked uploads in bytes, -1 means no limit" : "Bayt olarak düzenli arka plan taramaları ve parçalı yüklemeler için dosya boyutu sınırı. -1 sınır olmadığı anlamına gelir",
    "First bytes to scan" : "Taranacak ilk baytlar",
    "Check only first bytes of the file, -1 means no limit" : "Dosyanın yalnızca ilk baytını denetler. -1 yazılırsa sınırlama uygulanmaz",
    "Maximum stream length" : "En fazla akış uzunluğu",
    "Reopen socket after this number of bytes." : "Bu sayıda bayttan sonra soket yeniden açılır.",
    "Infected file handling" : "Virüslü dosya işleme",
    "When infected files are found during a background scan" : "Arka plan taraması sırasında virus bulaşmış dosyalar bulunduğunda",
    "Block uploads when scanner is not reachable" : "Tarayıcıya ulaşılamadığında yüklemeler engellensin",
    "Block unscannable files (such as encrypted archives)" : "Taranamayan dosyalar (şifrelenmiş arşivler gibi) engellensin",
    "Settings saved" : "Ayarlar kaydedildi",
    "Saving …" : "Kaydediliyor…",
    "Antivirus for Files" : "Dosyalar için Antivirus",
    "Clean" : "Temiz",
    "Infected" : "Enfekte",
    "Unchecked" : "Denetlenmemiş",
    "Scanner output" : "Tarayıcı çıktısı",
    "Greetings {user}," : "Merhaba {user},",
    "Sorry, but a malware was detected in a file you tried to upload and it had to be deleted." : "Ne yazık ki yüklemeye çalıştığınız dosyada zararlı bir malware yazılımı bulunduğu algılandığından silindi.",
    "This email is a notification from {host}. Please, do not reply." : "Bu ileti {host} tarafından otomatik olarak gönderildiğinden lütfen yanıtlamayın.",
    "File uploaded: {file}" : "Dosya yüklendi: {file}"
},
"nplurals=2; plural=(n > 1);");
